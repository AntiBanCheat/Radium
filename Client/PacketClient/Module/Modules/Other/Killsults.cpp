...
const char* Killsults::getModuleName() {
	...
	if (mode.getSelectedValue() == 6) name = string("Killsults ") + string(GRAY) + string("All");
	return name.c_str();
}
...
void Killsults::onPlayerTick(C_Player* plr) {
	...
	if (killed) {
		...
		case 6: // UwU
			random = rand() % 105;
			textPacket.message.setText(allMessage[random]);
			break;
	}
