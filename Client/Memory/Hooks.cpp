...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, C_Packet* packet) {
	...
	if (blinkMod->isEnabled() || freeTP->isEnabled() || (freecam->isEnabled() && freecam->cancelPackets) || speed->needblink) {
		if (!blinkMod->allpackets && packet->isInstanceOf<C_MovePlayerPacket>() || packet->isInstanceOf<PlayerAuthInputPacket>() || packet->isInstanceOf<C_PlayerActionPacket>()) {
			...
			if (blinkMod->isEnabled()) {
				if (packet->isInstanceOf<C_Packet>()) blinkMod->getPacketHolder()->push_back(new C_Packet(*reinterpret_cast<C_Packet*>(packet)));
			}
		}
	}
		...
		if (blinkMod->getPacketHolder()->size() > 0) {
			...
			blinkMod->getPacketHolder()->clear();
			return;
		}
	}
